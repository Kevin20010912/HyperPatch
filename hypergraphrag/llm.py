...
import ollama
import torch
from openai import AsyncOpenAI, APIConnectionError, RateLimitError, Timeout, AsyncAzureOpenAI, PermissionDeniedError
...
async def vllm_qwen3_8B_complete_if_cache(
    model,
    prompt,
    system_prompt=None,
    history_messages=[],
...
@retry(
    ...
    wait=wait_exponential(multiplier=1, min=8, max=10),
...
async def Qwen3_8B_complete(prompt, system_prompt=None, history_messages=[], keyword_extraction=False, **kwargs) -> str:
    keyword_extraction = kwargs.pop("keyword_extraction", None)

    return await vllm_qwen3_8B_complete_if_cache(
        "Qwen/Qwen3-8B",
        prompt,
        system_prompt=system_prompt,
        history_messages=history_messages,
...
async def openai_embedding(
    ...
    for i, t in enumerate(texts):
        if isinstance(t, str) and t.strip():
            repaired_texts.append(t.strip())
        elif isinstance(t, str):
            repaired_texts.append("EMPTY_INPUT")
            logger.warning(f"Empty string detected at index {i} value {t}, replaced with 'EMPTY_INPUT'.")
        elif t is None:
            repaired_texts.append("EMPTY_INPUT")
            logger.warning(f"None value detected at index {i} value {t}, replaced with 'EMPTY_INPUT'.")
        else:
            repaired_texts.append(str(t))
            logger.warning(f"Non-string input detected at index {i} value {t}, converted to string.")

    if not repaired_texts or all(x == "EMPTY_INPUT" for x in repaired_texts):
        repaired_texts = ["EMPTY_INPUT"]
        logger.warning("All input texts are empty or invalid. Replaced with a single 'EMPTY_INPUT' placeholder.")

    openai_async_client = AsyncOpenAI() if base_url is None else AsyncOpenAI(base_url=base_url)
    response = await openai_async_client.embeddings.create(model=model, input=repaired_texts, encoding_format="float")
...
_projector = None
_projector_device = "cpu"


def set_global_projector(projector: nn.Module, device: str = "cpu"):
    global _projector, _projector_device
    _projector = projector
    _projector_device = device


def get_global_projector():
    return _projector, _projector_device


@wrap_embedding_func_with_attrs(embedding_dim=256, max_token_size=8192)
@retry(
    stop=stop_after_attempt(9999),
    wait=wait_exponential(multiplier=1, min=8, max=60),
    retry=retry_if_exception_type(Exception),
)
async def openai2GNN_embedding(
    texts: list[str],
    model: str = "text-embedding-3-small",
    base_url: str = None,
    api_key: str = None,
    ...
    client = AsyncOpenAI() if base_url is None else AsyncOpenAI(base_url=base_url)
    response = await client.embeddings.create(model=model, input=repaired_texts, encoding_format="float")
    raw_embeddings = np.array([dp.embedding for dp in response.data])

    projector, device = get_global_projector()
    if projector is None:
        return raw_embeddings

    projector.eval()
    with torch.no_grad():
        input_tensor = torch.tensor(raw_embeddings, dtype=torch.float32, device=device)
        projected = projector(input_tensor).cpu().numpy()
        return projected
