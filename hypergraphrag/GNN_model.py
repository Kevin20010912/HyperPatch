...
class SentenceProjector(nn.Module):
    def __init__(self, sbert_dim, gnn_dim):
        super().__init__()
        self.linear = nn.Sequential(nn.Linear(sbert_dim, gnn_dim), nn.ReLU(), nn.LayerNorm(gnn_dim))
...
class GNN(torch.nn.Module):
    def __init__(self, text_emb_dim, gnn_input_dim, out_dim, activation, gnn_type="TransformerConv", gnn_layer_num=2):
        super().__init__()

        # build projector from 1536 to 256
        self.projector = SentenceProjector(text_emb_dim, gnn_input_dim)
        ...
        if gnn_layer_num < 1:
            ...
            self.conv = nn.ModuleList([GraphConv(gnn_input_dim, out_dim)])
        elif gnn_layer_num == 2:
            self.conv = nn.ModuleList([GraphConv(gnn_input_dim, 2 * out_dim), GraphConv(2 * out_dim, out_dim)])
        else:
            layers = [GraphConv(gnn_input_dim, 2 * out_dim)]
...
class GNNLoRA(torch.nn.Module):
    ...
    def forward(self, x, edge_index):
        ...
        final_emb = F.normalize(node_emb1 + node_emb2, dim=1)
        return final_emb, node_emb1, node_emb2
